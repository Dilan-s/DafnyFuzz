  ...
  protected ReturnStatus execute(Map<Variable, Variable> paramMap, StringBuilder s,
    ...
    if (values.isEmpty()) {
      ...
      variables.stream()
        ...
        .forEach(v -> {
          StringLiteral stringLiteral = new StringLiteral(new DString(true), symbolTable,
            v.getName());
    ...
    for (int i = 0; i < values.size(); i++) {
      ...
      if (types.stream().allMatch(Type::isPrintable)) {
        ...
        for (int j = 0; j < value.size(); j++) {
          ...
          if (newE != null) {
            OperatorExpression op = new OperatorExpression(symbolTable, new Bool(),
              BinaryOperator.Equals, List.of(exp, newE));
