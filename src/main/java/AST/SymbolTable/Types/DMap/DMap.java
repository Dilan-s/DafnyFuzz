...
public class DMap implements Type {
  ...
  public String formatEnsures(String variableName, Object object) {
    ...
    for (Entry<Object, Object> x : m.entrySet()) {
      String s = String.format("%s := %s", keyType.formatEnsures(x.getKey()),
        valueType.formatEnsures(x.getValue()));
      mapContents.add(s);
    }
  ...
  public boolean validMethodType() {
    return keyType.validMethodType() && keyType.isPrintable() && valueType.validMethodType()
      && valueType.isPrintable() && !keyType.equals(new DMap()) && !valueType.equals(
      new DMap());
  }
  ...
  public Boolean equal(Object lhsV, Object rhsV) {
    ...
    Set<Object> lhsKeys = lhsM.keySet();
    Set<Object> rhsKeys = rhsM.keySet();
    ...
    for (Object lhsK : lhsKeys) {
      for (Object rhsK : rhsKeys) {
        if (keyType.equal(lhsK, rhsK)) {
          Object lhsVa = lhsM.get(lhsK);
          Object rhsVa = rhsM.get(rhsK);
          if (!valueType.equal(lhsVa, rhsVa)) {
            return false;
          }
        }
      }
  ...
  public String formatPrint(Object object) {
    ...
    List<String> mapContents = new ArrayList<>();
    for (Entry<Object, Object> x : m.entrySet()) {
      String s = String.format("%s := %s", keyType.formatPrint(x.getKey()),
        valueType.formatPrint(x.getValue()));
      mapContents.add(s);
    }
  ...
  public boolean equals(Object obj) {
    ...
    if (keyType == null || dsetOther.keyType == null || valueType == null
      || dsetOther.valueType == null) {
      return true;
    }
