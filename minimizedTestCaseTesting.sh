...
javac -cp src/main/java/ -d ./out/ src/main/java/Main/MinimizedProgram.java src/main/java/Main/CompareOutputs.java
...
while [ true ]; do
    ...
    echo "Test number $x"
    timeout -s SIGKILL $t java -cp out/ Main.MinimizedProgram $x
    if [ $? -ne 0 ]
    then
	...
	echo "Test number $x" >> "errors/compErrors/$language.txt"
        ...
        cp tests-minimized/* "errors/$x/tests"
    fi
